# keep in sync with frontend/src/types.ts
...
- `prompt` must be at most 2500 characters.
...
def deserialize_block(block_json: dict[str, Any]) -> Block:
    ...
    if block_type == "llm_filter":
        ...
        _validate_prompt_length(prompt, label="llm_filter.prompt")
        ...
        batch_prompt = _optional_string(block_json, "batch_prompt", default="")
        batch_size = int(_optional_number(block_json, "batch_size", default=10))
        return LLMFilter(
            prompt=prompt,
            tier=tier,
            batch_prompt=batch_prompt or None,
...
def deserialize_condition(condition_json: dict[str, Any]) -> Condition:
    ...
    if condition_type == "llm":
        ...
        _validate_prompt_length(prompt, label="llm.prompt")
...
def _validate_prompt_length(prompt: str, *, label: str) -> None:
    if len(prompt) > MAX_LLM_PROMPT_CHARS:
        raise ValueError(f"{label} must be at most {MAX_LLM_PROMPT_CHARS} characters")
