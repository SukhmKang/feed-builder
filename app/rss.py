...
async def _parse_entry(
    ...
    *,
    ...
    if not content:
        raise ArticleParseError(f"Article {url} has no usable feed content")
    published_at = _extract_published_at(entry)

    full_text = await _extract_full_text_from_article_html(article_url=url)
    if not full_text:
        full_text = content
...
async def _extract_full_text_from_article_html(*, article_url: str) -> str:
    try:
        downloaded = await asyncio.to_thread(trafilatura.fetch_url, article_url)
    except Exception:
        return ""

    html = (downloaded or "").strip()
    if not html:
        return ""
